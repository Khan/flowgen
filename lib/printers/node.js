  ...
  switch (type.kind) {
    ...
    case ts.SyntaxKind.IndexSignature:
      if (type.type) {
        ...
        const readonlyMarker = ((_type$modifiers = type.modifiers) != null ? _type$modifiers : []).some(m => m.kind === ts.SyntaxKind.ReadonlyKeyword) ? "+" : "";
        return `${readonlyMarker}[${type.parameters.map(printers.common.parameter).join(", ")}]: ${printType(type.type)}`;
      }
